...
DeviceSpan<float> MarianState::Run(int current_length, DeviceSpan<int32_t>& next_tokens, DeviceSpan<int32_t> next_indices) {
  if(first_run_) {
    ...
    encoder_attention_mask_.name_ = model_.config_->model.encoder_decoder_init.inputs.attention_mask;
    ...
    *past_key_values_length_->GetTensorMutableData<int64_t>() = -1;
    ...
    auto encoder_hidden_states_shape = std::array<int64_t, 3>{decoder_input_ids_.GetShape()[0], encoder_input_ids_.GetShape()[1], 512};
    ...
    auto rnn_states_shape = std::array<int64_t, 3>{3, decoder_input_ids_.GetShape()[0], 512};
